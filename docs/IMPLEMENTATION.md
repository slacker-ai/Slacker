...
- **learnedPattern** — self-evolution (§7.5a). `id`, `channel_id` (nullable; NULL = global), `bucket` (enum: dismiss/ask/blocker/problem/help/decision/deadline), `phrase`, `status` (enum: proposed/approved/rejected/retired), `source` (enum: llm/manual), `rationale` (nullable), `supporting_label_count`, `created_at`, `decided_at` (nullable). Dismiss phrases are hard suppressions: they win over positive rules and do not create hidden item rows. Unique index on `(channel_id, bucket, phrase)`.
